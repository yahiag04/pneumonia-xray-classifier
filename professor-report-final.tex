...
media da circa 0.636 a circa 0.801, con un incremento medio di circa 0.165.
Un bootstrap paired sui cinque modelli, usati come unita' di confronto, stima
per questo delta medio un intervallo 95\% circa [0.123, 0.199]. Questo intervallo
non va interpretato come bootstrap sui singoli pazienti, perche' per la fase
Kermany-trained sono disponibili metriche aggregate e non predizioni campione per
campione; resta pero' una misura della stabilita' del miglioramento tra
architetture.
...
\midrule
PneumoniaNet & 0.634 & 0.830 & +0.197 \\
ResNet18 & 0.608 & 0.819 & +0.210 \\
MobileNetV3 & 0.618 & 0.797 & +0.179 \\
EfficientNet-B0 & 0.636 & 0.795 & +0.159 \\
DenseNet121 & 0.682 & 0.764 & +0.081 \\
\midrule
...
I risultati mostrano una saturazione netta della rete custom. Su RSNA, passare
da 75457 parametri a 4.78 milioni di parametri cambia la \balacc{} solo da
0.922 a 0.923, con massimo osservato 0.924 a \(w=3.0\). Quindi, nel subset
attuale, la capacita' aggiuntiva di PneumoniaNet non si traduce in un guadagno
interno proporzionale. Il punto \(w=1.0\), cioe' la rete originale, non e' il
massimo della curva ma e' gia' vicino alla saturazione; il risultato piu'
importante e' che l'aumento di larghezza non colma il divario con
EfficientNet-B0 su RSNA.

La curva EfficientNet conferma che architettura e pretraining contano piu' del
solo numero di parametri. EfficientNet-B0 ottiene la migliore \balacc{} su RSNA
(0.950) e Kermany (0.818) pur essendo la variante EfficientNet piu' piccola e
meno costosa. B1, B2 e B3 aumentano parametri e GMAC, ma non migliorano in modo
monotono la \balacc{}. Su Chittagong il miglior punto globale resta
PneumoniaNet \(w=3.0\), ma il vantaggio e' piccolo rispetto a EfficientNet-B2
(0.817 contro 0.805) e arriva con un costo computazionale molto piu' alto
(4.704 GMAC contro 0.658 GMAC).

Operativamente, questo studio suggerisce che il limite principale non e'
semplicemente la capacita' grezza del modello. Con il subset RSNA corrente, la
CNN custom satura presto; per migliorare servono architetture piu' efficienti,
pretraining piu' informativo, piu' dati o un protocollo di addestramento diverso.
La nuova opzione CLI \texttt{--train-size} permette di ripetere lo stesso studio
con piu' immagini RSNA, proprio per distinguere saturazione dovuta alla
capacita' da saturazione dovuta alla dimensione del training set.
...
Un meccanismo plausibile riguarda anche la proiezione AP/PA: il subset RSNA e'
quasi bilanciato tra PA e AP, mentre Kermany e' sostanzialmente dominato da
proiezioni AP pediatriche. RSNA puo' quindi coprire una porzione piu' ampia
della variabilita' di acquisizione, incluso il regime AP che rende meno brusco
il trasferimento verso Kermany; al contrario, un training quasi solo AP puo'
non coprire adeguatamente dataset adulti con maggiore presenza PA.
...
\paragraph{4. Il ribaltamento di DenseNet121 e' un effetto di calibrazione.}
DenseNet121 e' il modello migliore su Kermany e ha la \balacc{} piu' bassa su
Chittagong, ma la sua ROC-AUC su Chittagong resta competitiva (0.844): il potere
discriminativo non crolla. Lo split sensitivity/specificity in
Tabella~\ref{tab:benchmark-post-training} mostra
uno spostamento sistematico del punto operativo a soglia 0.5 verso la
specificity al crescere della capacita' del modello: si passa da sensitivity
0.875 e specificity 0.786 per PneumoniaNet a sensitivity 0.602 e specificity
0.926 per DenseNet121. Il ribaltamento in \balacc{} riflette quindi soprattutto
la posizione della soglia nel dominio Chittagong, non necessariamente un
...
\paragraph{8. Aumentare la capacita' non basta.}
Lo studio di scaling controllato mostra che la CNN custom satura rapidamente:
la curva PneumoniaNet resta intorno a 0.914--0.924 di \balacc{} su RSNA pur
passando da 75457 a 4782593 parametri. EfficientNet-B0, invece, ottiene 0.950
su RSNA e 0.818 su Kermany con soli 0.385 GMAC. Questo indica che il guadagno
non dipende solo dalla capacita' grezza, ma anche dal disegno architetturale e
dal pretraining.
...
\begin{itemize}
  ...
  complete multi-seed per stimare la variabilita' del training.
  ...
  cosi' lo stesso protocollo puo' essere rilanciato con piu' esempi.
  \item \textbf{Delta crossover}: l'intervallo riportato sul delta medio tra
  training Kermany e training RSNA, entrambi valutati su Chittagong, e' calcolato
  sulle cinque architetture come unita' paired. Non e' un bootstrap sui singoli
...
Lo studio di scaling controllato rafforza questa lettura. La rete custom scala
in modo regolare come numero di parametri e GMAC, ma la \balacc{} interna resta
quasi piatta: il miglior punto PneumoniaNet e' \(w=3.0\) con 0.924 su RSNA,
mentre \(w=1.0\) e' gia' a 0.914. EfficientNet-B0 raggiunge invece 0.950 su
RSNA e 0.818 su Kermany con meno di 0.4 GMAC. Quindi, nel subset corrente,
avvicinarsi alla top-performance richiede piu' della semplice larghezza della
CNN custom: contano architettura, pretraining e rapporto tra capacita' e dati
disponibili.
