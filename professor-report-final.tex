...
Chittagong. I valori assoluti delle due tabelle non devono quindi essere usati
come se fossero la stessa identica esecuzione; il seed controllato rende il
protocollo riproducibile, ma non garantisce una traiettoria bitwise identica su
backend stocastici. La tabella di scaling serve soprattutto a leggere
l'andamento interno delle curve addestrate nello stesso esperimento.
...
sulla generalizzazione esterna. Lo studio dimostra una saturazione interna
netta, ma non consente una conclusione forte sullo scaling esterno.
...
\paragraph{8. Lo scaling separa saturazione interna e rumore esterno.}
Lo studio di scaling controllato mostra una saturazione interna chiara: su RSNA
PneumoniaNet resta tra 0.914 e 0.924 di \balacc{} pur passando da 75457 a
4782593 parametri, mentre EfficientNet-B0 e' gia' il punto migliore della propria
famiglia. Sui dataset esterni, pero', le curve oscillano senza trend monotono.
...
rispetto alla capacita' per entrambe le famiglie: la performance satura prima
del limite esplorato. Sui dataset esterni, invece, le curve non sono monotone e
oscillano in un intervallo comparabile all'effetto atteso della capacita'. Con
un solo seed, quindi, lo studio non dimostra una legge di scaling esterna:
mostra che serve controllare la variabilita' prima di attribuire i cambiamenti
di generalizzazione alla complessita' del modello.
