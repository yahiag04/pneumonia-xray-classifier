...
RSNA_TO_MULTICLASS = {
    ...
    "No Lung Opacity / Not Normal": "not_normal_no_lung_opacity",
}
MULTICLASS_LABELS = tuple(RSNA_TO_MULTICLASS.values())


@dataclass(frozen=True)
class RsnaMulticlassRecord:
    patient_id: str
    label: str
    image_path: Path
    rsna_class: str
...
def map_rsna_class(rsna_class: str) -> str | None:
    return RSNA_TO_MULTICLASS.get(rsna_class.strip())


def collect_dicom_records(
    detailed_class_csv: str | Path,
    image_dir: str | Path,
) -> list[RsnaMulticlassRecord]:
    detailed_class_csv = Path(detailed_class_csv)
    image_dir = Path(image_dir)
    by_patient: dict[str, RsnaMulticlassRecord] = {}
    ...
    with detailed_class_csv.open(newline="") as handle:
        ...
        for row in reader:
            ...
            if not dicom_path.is_file():
                continue
            by_patient[patient_id] = RsnaMulticlassRecord(
...
def split_balanced_records(
    records: list[RsnaMulticlassRecord],
    train_per_class: int,
    val_per_class: int,
    test_per_class: int,
    seed: int = 42,
) -> dict[str, list[RsnaMulticlassRecord]]:
...
def write_split(
    ...
    for record in records:
        ...
        if record.source_format == "dicom":
            metadata = convert_dicom_to_png(record.image_path, output_path)
        elif record.source_format == "png":
            materialize_png(record.image_path, output_path, link_mode=link_mode)
            metadata = {
...
def prepare_rsna_multiclass(
    ...
    if detailed_csv.is_file() and image_dir.is_dir():
        source_layout = "dicom"
        records = collect_dicom_records(detailed_csv, image_dir)
    ...
    summary = {
        ...
        "total_multiclass_records_available": len(records),
        "labels": list(MULTICLASS_LABELS),
        "splits": {
            split: {
                "total": len(split_records),
                **{
                    label: sum(1 for record in split_records if record.label == label)
                    for label in MULTICLASS_LABELS
                },
            }
            for split, split_records in splits.items()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare a balanced 3-class ImageFolder from RSNA Pneumonia Detection Challenge."
    )
    parser.add_argument("--rsna-root", required=True)
    parser.add_argument("--output-root", default="data/rsna_multiclass_size_matched")
